...
export function getDefaultAgentMd(): string {
  ...
  const inboxFolder = t("mockData.inboxFolder");
  const projectsFolder = t("mockData.projectsFolder");
  const dailyNotesFolder = t("mockData.dailyNotesFolder");

  return `---
language: "${language}"
${getKey("name").toLowerCase()}: "${t("exampleAgent.name")}"
${getKey("description").toLowerCase()}: "${t("exampleAgent.description")}"
...
export function getBasicAgentMd(): string {
  ...
  return `---
language: "${language}"
${getKey("name").toLowerCase()}: "${t("basicAgent.name")}"
${getKey("readPermissions").toLowerCase()}:
  - "/"
${getKey("vaultRootAccess").toLowerCase()}: true
---

${t("basicAgent.promptBody")}`;
}
  ...
  static async generateMockData(
    ...
    try {
      ...
      const ideaCreated = await this.createFileIfNotExists(
        app,
        `${inboxFolder}/${t("mockData.ideaTitle")}`,
      ...
      const projectCreated = await this.createFileIfNotExists(
        app,
        `${projectsFolder}/${t("mockData.projectTitle")}`,
      ...
      if (generatedCount > 0) {
        ...
        new Notice(t("notices.mockDataExists"));
      }
    } catch (e) {
      new Notice(t("notices.mockDataFailed", { message: String(e) }));
    }
  }
