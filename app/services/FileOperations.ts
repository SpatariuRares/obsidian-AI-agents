...
export class FileOperations {
     ...
     */
    static async writeFile(app: App, config: AgentConfig, path: string, content: string, mode: "overwrite" | "append" | "prepend" = "overwrite"): Promise<void> {
     ...
     */
    static async createFile(app: App, config: AgentConfig, path: string, content: string): Promise<void> {
        PermissionGuard.assertPermission(config, "create", path);

        if (config.confirm_destructive) {
            const allowed = await FileOperations.requestUserConfirmation(app, config.name, "create", path);
            if (!allowed) throw new Error("Operation rejected by user.");
        }
     ...
     */
    static async moveFile(app: App, config: AgentConfig, fromPath: string, toPath: string): Promise<void> {
    ...
    static async deleteFile(app: App, config: AgentConfig, path: string): Promise<void> {
        ...
        if (config.confirm_destructive) {
            const allowed = await FileOperations.requestUserConfirmation(app, config.name, "delete", path);
            if (!allowed) throw new Error("Operation rejected by user.");
        }
     ...
     */
    static async listFiles(app: App, config: AgentConfig, path: string, recursive: boolean = false): Promise<string[]> {
        PermissionGuard.assertPermission(config, "read", path);
     ...
     */
    private static async requestUserConfirmation(app: App, agentName: string, operation: FileOperationType, path: string): Promise<boolean> {
