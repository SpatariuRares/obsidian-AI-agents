 ...
 * t('settings.title'); // "Plugin Settings"
 * t('notices.exampleNotice'); // "Example notice fired!"
 * ```
 */
...
export class LocalizationService {
  ...
  /**
   * Detect user's language from Obsidian's bundled moment locale.
   * Obsidian sets window.moment.locale() to match the user's chosen interface
   * language, so reading it gives us the correct locale without any extra deps.
   * Falls back to 'en' when the locale cannot be determined.
   *
   * --- How to add a new language ---
   * 1. Create app/i18n/locales/<locale>.json  (e.g. de.json for German)
   * 2. Copy en.json and translate all values — keep the key structure identical
   * 3. That's it. The service automatically loads the file when the user's
   *    Obsidian is set to that language; en.json is always the fallback.
   */
